...
Spring Boot 4.0 removed the `@MockBean` and `@SpyBean` annotations from `org.springframework.boot.test.mock.mockito`. Code compiles on Spring Boot 3.5.16 but fails at runtime on 4.0 because the annotation classes no longer exist. The annotations have been replaced with Mockito's native bean override mechanism: `@MockitoBean` and `@MockitoSpyBean` in the `org.springframework.test.context.bean.override.mockito` package.
...
The supporting beans (MyService, HelperService) are simple Spring-managed services. The test runs in a `@SpringBootTest` context, which on 3.5.16 correctly injects the mocked/spied beans; on 4.0, the annotations are not recognized.
