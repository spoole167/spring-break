...
- **pom.xml**: Spring Boot 3.5.16 parent with `spring-boot-starter-batch` and h2
...
- **batchJobSeqTableShouldExist()**: Queries for BATCH_JOB_SEQ table. Spring Boot 3.5.16 auto-initializes with this name; Spring Boot 4.0 auto-initializes with BATCH_JOB_INSTANCE_SEQ instead.
- **batchStepExecutionSeqTableShouldExist()**: Queries for BATCH_STEP_EXECUTION_SEQ table. Same pattern: old name on 3.5.16, renamed on 4.0.

The test demonstrates the breaking change by verifying the old schema is present (passes on 3.5.16, fails on 4.0).
